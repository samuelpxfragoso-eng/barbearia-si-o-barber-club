...
const Services: React.FC = () => {
  const categories = [
    {
      title: 'CABELO',
      icon: <Scissors size={18} className="text-[#c2a440]" />,
      items: [
    ...
    {
      title: 'BARBA',
      icon: <Zap size={18} className="text-[#c2a440]" />,
      items: [
    ...
    },
    {
      ...
      icon: <Sparkles size={18} className="text-[#c2a440]" />,
      items: [
    ...
    },
    {
      ...
      icon: <Star size={18} className="text-[#c2a440]" />,
      items: [
  ...
  return (
    <section id="servicos" className="py-24 bg-[#0b0d11] relative overflow-hidden">
      {/* Background accents */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#c2a440]/5 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="mb-16">
          ...
          <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tighter leading-tight mb-8">
            TABELA DE <span className="text-[#c2a440]">SERVIÇOS.</span>
          </h3>
          <p className="text-white/40 max-w-2xl text-lg font-medium leading-relaxed">
            Oferecemos uma curadoria completa de serviços focada na estética masculina contemporânea, unindo técnicas clássicas à inovação constante de nossa equipe.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-[1200px] mx-auto">
          {categories.map((cat, idx) => (
            <div key={idx} className="bg-black/40 border border-white/5 backdrop-blur-md p-8 md:p-10 rounded-[32px] transition-all hover:border-[#c2a440]/20 group">
              <div className="flex items-center gap-3 mb-10">
                <div className="w-8 h-8 rounded-lg bg-[#c2a440]/10 flex items-center justify-center border border-[#c2a440]/20">
                  {cat.icon}
                </div>
                <h4 className="text-lg font-black uppercase tracking-widest text-white group-hover:text-[#c2a440] transition-colors">
                  {cat.title}
                </h4>
              </div>
                ...
                {cat.items.map((item, iIdx) => (
                  <div key={iIdx} className="flex flex-col gap-1 group/item">
                    <div className="flex items-end justify-between">
                      <span className={`text-sm md:text-base font-bold ${item.highlight ? 'text-[#c2a440]' : 'text-white/90'}`}>
                        {item.name}
                      </span>
                      <div className="flex-1 border-b border-white/5 mx-4 mb-1.5" />
                      <span className={`text-sm md:text-base font-black ${item.highlight ? 'text-[#c2a440]' : 'text-white/70'}`}>
                        {item.price}
                      </span>
                    </div>
                    {item.subtitle && (
                      <span className="text-[10px] font-black text-[#c2a440]/50 uppercase tracking-widest">
                        {item.subtitle}
                      </span>
                    )}
                  </div>
        ...
        <div className="mt-20 text-center">
          ...
          <a 
            ...
            className="inline-flex items-center gap-3 bg-[#d4b550] hover:bg-[#e6c762] text-black px-12 py-5 rounded-2xl font-black uppercase tracking-widest text-xs md:text-sm transition-all gold-glow group"
          >
